...
export default {
  ...
  getExpensesByMonth({ commit }, params) {
    ...
    return Vue.prototype.$http('/expenses/byMonth', { params, headers })
      .then(response => {commit('SET_EXPENSES', response.data), console.log('response =>', response)})
      .catch(error => {
        return error
      })
      .finally(() => commit('SET_PRELOADER', false))
  ...
  deleteExpense({ commit }, params) {
    ...
    return Vue.prototype.$http.delete('/expenses/delete', { data: { id: params.id, expense_id: params.expense_id }, headers })
